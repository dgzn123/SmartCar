...
void TFT2SPI_Init(unsigned char type)
{
  ...
  if(type==0)// ===== rotate 0 degree ===== //  屏排线与ＩＣ在下方OK
    {
      TFT2mode=0;
      write_command(0x02,0x0100);
      write_command(0x03,0x1030);
...
void TFT2SPI_Write_Cmd(unsigned char cmd)
{
  ...
  for(i=0;i<8;i++)
  {
    TFT2SPI_SCK_L;TFT2delay_1us(1);	// SCK=0  ILI9163_SCK=0;
    if(cmd&0x80){TFT2SPI_SDI_H;}// SDI=1
    else{TFT2SPI_SDI_L;}// SDI=0
    TFT2SPI_SCK_H;TFT2delay_1us(1);	// SCK=1  ILI9163_SCK = 1;
    cmd = (cmd<<1);
  }
}
...
void TFT2SPI_Write_Byte(unsigned char dat)
{
  ...
  {
    TFT2SPI_SCK_L;TFT2delay_1us(1);	// SCK=0  ILI9163_SCK=0;
    if(dat&0x80){TFT2SPI_SDI_H;}// SDI=1
    else{TFT2SPI_SDI_L;}// SDI=0
    TFT2SPI_SCK_H;TFT2delay_1us(1);	// SCK=1 ILI9163_SCK = 1;
    dat = (dat<<1);
  }
...
void TFT2SPI_Write_Word(unsigned short dat)
{
  ...
  {
    TFT2SPI_SCK_L;TFT2delay_1us(1);	// SCK=0  ILI9163_SCK=0;
    if(dat&0x8000){ TFT2SPI_SDI_H;}// SDI=1
    else{TFT2SPI_SDI_L;}// SDI=0
    TFT2SPI_SCK_H;TFT2delay_1us(1);	// SCK=1  ILI9163_SCK=1;
    dat<<=1;
  }
...
* @note  内部调用
*
* @see   TFT2SPI_P8X8(1, 1, "123456", u16YELLOW, u16RED);
*
* @date  2019/6/13 星期四
*/
...
{
  while(*s_dat) TFT2SPI_P6X8(x++, y, *s_dat++,word_color,back_color);
}
...
{
  while(*s_dat) TFT2SPI_P8X8(x++, y, *s_dat++,word_color,back_color);
}
...
{
  while(*s_dat) TFT2SPI_P8X16(x++, y, *s_dat++,word_color,back_color);
}
...
void TFT2SPI_Show_Title(void)
{
  ...
  TFT2SPI_Draw_Line(0,24,160,24,u16BLACK);   //补齐分割线
}
...
void Test_TFT2(void)
{
  ...
  while(1)
  {
    ...
    TFT2SPI_P8X16Str(0, 6, txt, u16RED, u16BLUE);     //将txt中 内容显示出来
